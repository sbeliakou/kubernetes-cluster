...
if [ "${MODE}" == "master" ] && [ -d /etc/kubernetes/manifests ]; then
    ...
    kubeadm init --apiserver-advertise-address ${IPADDR} --token ${TOKEN} --pod-network-cidr 10.244.0.0/16 2>&1 | tee /tmp/kubeadm.init.log

    cat /tmp/kubeadm.init.log | grep "kubeadm join" > /vagrant/worker-join.sh
